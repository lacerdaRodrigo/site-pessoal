...
**Template do e-mail de confirmação (decidido em 2026-07-05):** o template padrão do Supabase ("Confirm your email address", em inglês, sem identidade do produto) foi substituído por um texto em português com a marca "Knowledge Hub", editado diretamente em **Authentication → Email Templates → Confirm signup** no painel. Não foi configurado SMTP customizado (o remetente continua sendo o do Supabase, `noreply@mail.app.supabase.io`) — trocar isso exigiria um domínio de e-mail próprio, fora de escopo por ora.

...
- **🚧 Pendente (2026-07-05) — secrets do E2E no GitHub:** o workflow de E2E (`.github/workflows/e2e.yml`) precisa de mais dois secrets no repositório (`Settings → Secrets and variables → Actions → New repository secret`): `E2E_EMAIL` e `E2E_SENHA`, com os mesmos valores do `.env.test.local` (conta de teste já confirmada no Supabase). Sem eles o workflow **não fica vermelho** — os casos que exigem login são pulados (rodam só CT-19/CT-20) — mas a cobertura completa (CT-10 a CT-13) só entra com os secrets. Os secrets do CI de qualidade (`NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_ANON_KEY`) já foram adicionados e o CI está verde ✅.
- **🚧 Pendente (2026-07-06) — completar a branch protection da `main` (ADR 11):** a regra foi criada, mas (1) faltou marcar as três opções da seção 8 de `docs/07-deploy/01-ambientes-e-pipeline.md` (*Require a pull request*, *Require status checks* com os dois checks, *Do not allow bypassing*), e (2) descobriu-se que o GitHub Free **não aplica** a regra em repositório privado — por isso o repositório está sendo tornado público (ver "Repositório Público" em Decisões Tomadas). Ordem: tornar público → editar a regra e marcar as três opções → salvar. Aí a trava passa a valer de fato.

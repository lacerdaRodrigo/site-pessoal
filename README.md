...
- **🚧 Pendente (2026-07-06) — ativar Dependabot (alerts) + CodeQL no painel do GitHub:** o arquivo `.github/dependabot.yml` (atualizações de versão / *version updates*) já está versionado no repositório, mas os **alertas de vulnerabilidade** e o **CodeQL** são toggles manuais em `Settings → Code security`: ativar *Dependabot alerts*, *Dependabot security updates* e *Code scanning → CodeQL (Default setup)*. Agora que o repositório é público, os três são gratuitos. Ver `docs/07-deploy/01-ambientes-e-pipeline.md`, seção 2.

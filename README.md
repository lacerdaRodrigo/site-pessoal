...
| CRUD de Documentos (criar, ler, editar, excluir) | 🚧 planejamento pausado — ver Próximos Passos |
| Editor Markdown | ⬜ |
...
12. 🚧 **CRUD de Documentos (planejamento PAUSADO em 2026-07-06 — IMPEDIMENTO para retomar):** a modelagem chegou a ser iniciada (tabela `documentos`, ver `docs/02-banco-de-dados/01-entidades.md`), mas foi pausada de propósito para seguir a Regra de Ouro "Pendências do Git Primeiro" — havia 3 PRs do Dependabot abertos (#3/#4/#5). **Retomar assim que esses PRs estiverem mergeados.** Ao voltar: modelar `documentos` (depende de `perfis` + `categorias` por FK, então a 1ª leva são 3 migrations nessa ordem, cada uma já com RLS), primeira migration via Supabase CLI (⚠️ o CLI ainda não instalou via `npx` nesta máquina — resolver isso faz parte de retomar), Server Actions de CRUD, telas e testes nos 3 níveis. É a próxima fase de código e destrava Busca/Favoritos/Categorias/Dashboard.
